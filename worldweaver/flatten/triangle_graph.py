...
from worldweaver.flatten.utils import get_bottlenecks, split_triangles_with_bottlenecks
...
def get_triangle(p: Polygon, get_elevation: Callable[[list[float]], float]) -> Polygon:
    return Polygon(
        [(c[0], c[1], get_elevation([c[0], c[1]])) for c in p.exterior.coords]
    )
...
def get_triangle_graph_as_nx(
    gdf_triangle: gpd.GeoDataFrame, id_column: str = "triangle_id"
) -> nx.Graph:
    touching_triangles = gdf_triangle.sjoin(gdf_triangle, predicate="touches")
    graph = nx.Graph()
    for triangle_id_left, triangle_id_right in zip(
        touching_triangles[f"{id_column}_left"],
        touching_triangles[f"{id_column}_right"],
    ):
