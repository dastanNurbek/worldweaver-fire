...
from worldweaver.flatten.triangle_graph import get_triangle_graph_as_nx
...
def optimize_all_segments(
    ...
    :type gamma: float
    """
    logger.info(f"optimize_all_segments - building data structure")
    ...
    logger.info(f"optimize_all_segments - triangles")
    ...
    logger.info(f"triangles_idx {len(triangles_idx)}")
    logger.info(f"optimize_all_segments - adjacency")
    ...
    adjacency = [
        (triangle_map[u], triangle_map[v])
        for u, v in graph.edges
        if (u in triangle_map) and (v in triangle_map)
    ]
    logger.info(f"optimize_all_segments - creating problem")
    # original heights
    ...
    try:
        logger.info(f"optimize_all_segments - solve")
