...
from worldweaver.flatten.elevation import interpolate, throttle_requests
from worldweaver.flatten.optimize import optimize_all_segments
from worldweaver.flatten.orient_triangle_graph import get_oriented_graph
from worldweaver.flatten.triangle_graph import (
    get_triangles_and_segments,
    remove_interstitial_nodes,
    reverse,
)
from worldweaver.flatten.wfs import get_hydro_data
...
def build_triangles_and_points(
    ...
    logger.info(
        f"{len(gdf_triangle)} triangles, {len(gdf_triangle_segment)} triangle segments"
    )
    ...
    logger.info(f"build_triangles_and_points - all done!")
...
def process_all(
    ...
    res = build_triangles_and_points(
        union, segments, max_segment_length, get_elevation, output_file
    )
    if res is None:
    ...
    logger.info(
        f"optimize_all_segments ({len(points)} points, {len(triangles)} triangles)"
    )
    points = optimize_all_segments(
        points,
        triangles,
        alpha=alpha,
        beta=beta,
    ...
    if points is None:
        ...
        return None
    logger.info(f"optimize triangles ({len(triangles)})")
    final_triangles_gdf = optimize_triangles(points, triangles)
    logger.info(f"optimized triangles ({len(final_triangles_gdf)})")
    if output_file:
...
def optimize_triangles(
    points: gpd.GeoDataFrame, triangles: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    """
...
if __name__ == "__main__":
    ...
    res = main(
        srs, box, max_segment_length, alpha, beta, gamma, delta, epsilon, output_file
    )
